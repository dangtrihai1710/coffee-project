...
class AgentSystem {
  ...
  static async coordinateAgents(userInput, context = {}) {
    try {
      ...
      // Thông tin về bệnh và kỹ thuật
        ...
        response = await this.provideDiseaseAndTechniqueInfo(userInput, context, intent);
      }
  ...
  /**
   * Phân tích kết quả quét lá cà phê
  ...
  /**
   * Cung cấp thông tin về bệnh và kỹ thuật
   * @param {string} userInput - Yêu cầu của người dùng
   * @param {object} context - Ngữ cảnh
   * @param {object} intent - Ý định đã phân tích từ trước 
   * @returns {object} - Phản hồi chi tiết
   */
  static async provideDiseaseAndTechniqueInfo(userInput, context = {}, intent = null) {
    try {
      if (!intent) {
      ...
      console.error('Lỗi khi tạo thông tin về bệnh và kỹ thuật:', error);
      return {
  ...
  static _analyzeUserIntent(userInput) {
    ...
    if (Object.values(KEYWORDS.SCAN).some(k => lowerInput.includes(k))) {
      topics.push('scan');
    }
